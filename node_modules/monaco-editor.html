...
<body style="background-color:black;" onload="requestCurrentPageStatus()">
    ...
    <p id="changeStyleButton" onclick="changeStyle()">🌜</p>
        ...
        function saveJs() {
            if (!dataBeingSent) {
                ...
                if (fileName == "") {
                    ...
                    var numberOf = fileName.replace(/[^/]/g, "").length;
                    ws.onopen = function () {
                        ws.send(JSON.stringify({
                            action: "saveFile",
                            fileName: fileName.split("/")[numberOf].split(":")[0],
                            fileData: getVal
                        }));
                        ws.close();
                    }
                    ws.onclose = function () {
                        dataBeingSent = false;
                        alert(fileName.split("/")[numberOf].split(":")[0] + " Saved");
                    }
                } else if (fileName.split(":")[0] == "http" || fileName.split(":")[0] == "https") {
                    alert("Saving To http Or https Server Is Not Supported");
                } else {
                    var messageData = {
                        action: "save",
